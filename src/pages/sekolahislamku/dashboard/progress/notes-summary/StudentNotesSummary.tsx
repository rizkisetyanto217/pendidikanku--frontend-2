...
function makeNotes(days = 30): NoteLog[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    if (i % 5 === 0) base.nilai = 85 + (i % 3) * 3; // 85, 88, 91
    if (i % 3 === 0)
  ...
  return (
    ...
    >
      <PageTopBar
        palette={palette}
        onBack={() => navigate(-1)}
        label="Riwayat Catatan & Hafalan"
        title={`${s?.student.name ?? "—"} • ${s?.student.className ?? "—"}`}
      />
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 space-y-6">
          ...
          {s && (
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              ...
              >
                ...
                <Badge className="mt-1" variant="secondary" palette={palette}>
                  {s.stats.withPR}
                </Badge>
              </SectionCard>
              <SectionCard
        ...
        <SectionCard palette={palette} className="p-4 md:p-5">
          <div className="font-medium mb-3 flex items-center gap-2">
            <NotebookPen size={18} color={palette.quaternary} /> Daftar Catatan
          </div>
          ...
          <div className="pt-6 flex items-center justify-between gap-3">
            <Link to="/student/student-progress">
