...
/* ============ Query: Lembaga Stats (v5-friendly) ============ */
function useLembagaStats() {
  const q = useQuery<LembagaStats | null>({
    queryKey: ["lembaga-stats"],
    queryFn: async () => {
      const res = await axios.get<LembagaStatsResponse>("/api/a/lembaga-stats");
      console.log("🏷️ Lembaga stats berhasil:", res.data);
  ...
  // Logging (gantikan onSuccess/onError)
  useEffect(() => {
    if (q.status === "success") {
      console.debug("[Q] lembaga-stats OK", {
        hasData: !!q.data,
        students: q.data?.lembaga_stats_active_students,
        teachers: q.data?.lembaga_stats_active_teachers,
      });
    }
  }, [q.status, q.data]);
  ...
  useEffect(() => {
    if (q.status === "error") {
      const err: any = q.error;
      console.error("[Q] lembaga-stats ERR", {
...
/* ============ Query: Today Sessions (v5-friendly) ============ */
function useTodaySessions() {
  const today = yyyyMmDdLocal();

  const q = useQuery<SessionsItem[]>({
    queryKey: ["class-attendance-sessions", "today", today],
    queryFn: async () => {
      console.debug("[Q] today-sessions REQ", { today });
      const res = await axios.get<SessionsResponse>(
        "/api/u/class-attendance-sessions",
  ...
  useEffect(() => {
    if (q.status === "success") {
      console.debug("[Q] today-sessions OK", { count: q.data?.length ?? 0 });
    }
  }, [q.status, q.data]);
  ...
  useEffect(() => {
    if (q.status === "error") {
      const err: any = q.error;
      console.error("[Q] today-sessions ERR", {
        status: err?.response?.status,
        message: err?.message,
        payload: err?.response?.data,
      });
