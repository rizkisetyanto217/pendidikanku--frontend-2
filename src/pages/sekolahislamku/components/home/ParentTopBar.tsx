...
function NavLinkItem({
  ...
  return (
    <NavLink to={to} end={!!end} onClick={onClick} className="block focus:outline-none">
      {({ isActive }) => (
        <div
          className="flex items-center gap-3 rounded-xl px-3 py-2 border transition-all hover:translate-x-px"
          style={{
...
function MobileDrawer({
  ...
  return (
    <div className="md:hidden" style={{ pointerEvents: "auto" }}>
      {/* overlay */}
      <div className="fixed inset-0 z-50" onClick={onClose} style={{ background: "#0006" }} />
      {/* panel */}
      <aside
        id="mobile-drawer"
        className="fixed inset-y-0 left-0 z-50 w-72 max-w-[70vw]"
        style={{ background: palette.white1, borderRight: `1px solid ${palette.silver1}` }}
        role="dialog"
        aria-label="Menu navigasi"
      >
        <div className="flex items-center justify-between px-3 py-3 border-b" style={{ borderColor: palette.silver1 }}>
          <div className="flex items-center gap-2">
            <GraduationCap size={20} color={palette.primary} />
            <span className="font-semibold">{brand}</span>
          </div>
        ...
        <div className="px-3 pt-3 pb-2 text-xs flex flex-col gap-1 items-start" style={{ color: palette.silver2 }}>
          {formattedGregorian}
        ...
        <nav className="px-2 pb-4" aria-label="Navigasi">
          <ul className="space-y-2">
            {navs.map(({ path, label, icon, end }) => (
              <li key={path}>
                <NavLinkItem to={path} icon={icon} label={label} end={end} palette={palette} onClick={onClose} />
              </li>
            ))}
          </ul>
        </nav>
...
export default function ParentTopBar({
  ...
  // navs
  const navs = useMemo<NavItem[]>(() => {
    const source = pageKind === "sekolah" ? SCHOOL_NAVS : pageKind === "guru" ? TEACHER_NAVS : STUDENT_NAVS;
    return source.map(({ path, label, icon, end }) => ({
      path: path === "." ? base : `${base}/${path}`,
      label,
      icon,
  ...
  const now = useMemo(() => new Date(), [pathname, midnightTick]);
  const gIso = gregorianDate ?? now.toISOString();
  const formattedGregorian = useMemo(() => (dateFmt ? dateFmt(gIso) : formatIDGregorian(gIso)), [gIso, dateFmt]);
  const hijriLabel = useMemo(() => hijriDate || formatHijriLocal(now), [hijriDate, now]);
  ...
  useEffect(() => {
    const d = new Date();
    const msUntilMidnight = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1).getTime() - d.getTime();
    const t = setTimeout(() => setMidnightTick((x) => x + 1), msUntilMidnight + 1000);
    return () => clearTimeout(t);
  }, [midnightTick]);

  const brandNode =
    title ??
    (pageKind === "sekolah" ? "Dashboard Sekolah" : pageKind === "guru" ? "Dashboard Guru" : "Dashboard Murid");
  ...
  return (
    <>
      ...
      <div
        className="sticky top-0 z-40 backdrop-blur border-b"
        style={{ background: `${palette.white1}E6`, borderColor: palette.silver1 }}
      >
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between gap-3">
          {/* Left */}
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="leading-tight">
                ...
                {/* Gregorian (>= sm) */}
                <div className="hidden sm:block text-xs" style={{ color: palette.silver2 }}>
                  {formattedGregorian}
                </div>
          ...
          {/* Right (desktop) */}
          <div className="hidden md:flex items-center gap-3" style={{ fontSize: 14, color: palette.silver1 }}>
            <span
              className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium"
              style={{
                background: palette.secondary,
