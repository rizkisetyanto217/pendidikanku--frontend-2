...
interface ParentTopBarProps {
  palette: Palette;
  /** Bila tidak diberikan, Masehi & Hijriah tetap dihitung lokal */
  hijriDate?: string;
...
const buildBase = (slug: string | undefined, root: "sekolah" | "murid") =>
  slug ? `/${slug}/${root}` : `/${root}`;
...
export default function ParentTopBar({
  ...
  const base = isSchoolPage
    ? buildBase(slug, "sekolah")
    : buildBase(slug, "murid");
  const navs = useMemo(
    () =>
      (isSchoolPage ? SCHOOL_NAVS : STUDENT_NAVS).map(
        ({ path, label, icon, end }) => ({
  ...
  // Auto update after midnight (so tanggal berganti tanpa reload)
  useEffect(() => {
    const d = new Date();
  ...
  const brandNode = title ?? "Dashboard Sekolah";
...
{/* Jdudul seblah j=kiri atas */}
          ...
          {/* Right (desktop): ALWAYS show Gregorian + Hijri */}
          <div
          ...
          {/* Right (mobile): hanya user menu; tanggal ditaruh di bawah */}
        ...
        <div className="sm:hidden px-4 pb-3">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <span className="text-sm font-medium">{formattedGregorian}</span>
            </div> */}
            {/* <span
              className="ml-2 inline-flex items-center rounded-full px-2 py-0.5 text-sm"
              style={{ background: palette.secondary, color: palette.white1 }}
            >
              {hijriLabel || "—"}
            </span> */}
