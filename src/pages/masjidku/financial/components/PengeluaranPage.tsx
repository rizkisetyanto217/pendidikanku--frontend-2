      ...
      {items.map((item, i) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <div style={{ color: theme.error1 }}>{item.icon}</div>
          </div>
          <div
...
const PengeluaranPage: React.FC = () => {
  ...
  const { data: statsResp, isLoading: loadingStats } = useQuery<{
    ...
    queryFn: async () =>
      (
        await axios.get(`/public/masjids/${slug}/financials/expense/stats`, {
          params: { month },
        })
      ).data,
    enabled: !!slug && !!month,
  ...
  const {
    ...
    isError: errorExpense,
  } = useQuery<{ data: ExpenseTransaction[] }>({
    queryKey: ["expense-transactions", slug, month],
    queryFn: async () =>
      (
        await axios.get(`/public/masjids/${slug}/financials/expense`, {
          params: { month },
        })
      ).data,
    enabled: !!slug && !!month,
